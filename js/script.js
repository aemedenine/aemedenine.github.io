...
function updateUserVisits(user) {
  ...
  userRef.transaction(current => {
    ...
    if(!err && committed && snap){
      ...
      if(rankEl){
        const starsCount = data.rank === "Member" ? 1 : data.rank === "Active" ? 2 : data.rank === "Pro" ? 3 : 5;
...
function updateOnlineUsers(user) {
  ...
  userOnlineRef.set({ avatar: user.photoURL || "", name: user.displayName || "", lastActive: firebase.database.ServerValue.TIMESTAMP });
...
function updateAverageStars() {
  ratingsRef.once('value').then(snap => {
    ...
    stars.forEach((star, i) => {
      star.classList.toggle('average-highlight', (i+1) <= Math.round(avg));
    });
  });
}
